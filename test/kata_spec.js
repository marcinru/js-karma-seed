...
describe("getQuote", function () {
  var qq = ["WHERE IS SHE?!", "Holy haberdashery, Batman!", "Let's put a smile on that faaaceee!"];
  it("returns a Robin's quote", function() {
    expect(getQuote(qq, "Rob1n")).toBe("Robin: Holy haberdashery, Batman!");
  });
  it("returns a Joker's quote", function() {
    expect(getQuote(qq, "Joke2")).toBe("Joker: Let's put a smile on that faaaceee!");
  });
  it("returns a Batman's quote", function() {
    expect(getQuote(qq, "Batm0n")).toBe("Batman: WHERE IS SHE?!");
  });
});